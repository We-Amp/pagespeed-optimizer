...
// Ported from mod_pagespeed's pagespeed/kernel/image/png_optimizer.cc
//
...
extern "C" {
#include "src/opngreduc/opngreduc.h"
#include "zlib.h"
}
...
// we use these four combinations because different images seem to
// benefit from different parameters and this combination of 4 seems
// to work best for a large set of PNGs from the web.
...
void PngErrorFn(png_structp png_ptr, [[maybe_unused]] png_const_charp msg) {
...
void PngWarningFn(png_structp png_ptr, [[maybe_unused]] png_const_charp msg) {
  PS_DLOG_INFO(static_cast<MessageHandler*>(png_get_error_ptr(png_ptr)),
...
// big-endian bytes.
inline uint32_t ReadUint32FromBigEndianBytes(const unsigned char* read_head) {
  return (static_cast<uint32_t>(*read_head) << 24) +
         (static_cast<uint32_t>(*(read_head + 1)) << 16) +
         (static_cast<uint32_t>(*(read_head + 2)) << 8) +
         static_cast<uint32_t>(*(read_head + 3));
}
...
PngCompressParams::~PngCompressParams() = default;
...
bool ScopedPngStruct::reset() {
  ...
  if (png_ptr_ == nullptr) {
    PS_LOG_DFATAL(message_handler_, "Failed to create libpng struct.");
    return false;
  }

  // png_create_info_struct returns NULL on failure (no longjmp), so call it
  // directly rather than wrapping in PngProtectedCall.
  info_ptr_ = png_create_info_struct(png_ptr_);
...
PngReaderInterface::PngReaderInterface() = default;
...
PngOptimizer::~PngOptimizer() = default;
...
bool PngOptimizer::CreateOptimizedPng(const PngReaderInterface& reader,
  ...
  // Copy the image data from the read structures to the write
  // structures.
  if (!CopyReadToWrite()) {
    return false;
  }
...
bool PngOptimizer::CreateBestOptimizedPngForParams(
  ...
  for (size_t idx = 0; idx < param_list_size; ++idx) {
    ...
    // libpng doesn't allow for reuse of the write structs, so we must
    // copy on each iteration of the loop.
    CopyPngStructs(write_, &write);
    if (CreateOptimizedPngWithParams(
            &write,
            param_list  // NOLINT(bugprone-pointer-arithmetic-on-polymorphic-object)
                [idx],
            &temp_output)) {
...
bool PngReader::ReadPng(const std::string& body, png_structp png_ptr,
...
bool PngReader::GetAttributes(const std::string& body, int* out_width,
  ...
  const auto* read_head = reinterpret_cast<const unsigned char*>(body.data());
  ...
  const uint32_t first_chunk_len = ReadUint32FromBigEndianBytes(read_head);
...
bool PngOptimizer::WritePng(ScopedPngStruct* write, std::string* buffer) {
  return PngProtectedCall(write->png_ptr(), [&] {
...
bool PngOptimizer::CopyPngStructs(const ScopedPngStruct& from,
  ...
  // double-free. In libpng 1.6+, free_me is always enabled (the
  // PNG_FREE_ME_SUPPORTED macro was removed because it's no longer
  // optional).
...
bool PngReaderInterface::IsAlphaChannelOpaque(png_structp png_ptr,
  ...
  if ((color_type & PNG_COLOR_MASK_ALPHA) == 0) {
    ...
    PS_LOG_DFATAL(handler,
                  "IsAlphaChannelOpaque called for image without alpha "
                  "channel.");
    return false;
  }
  ...
  if (color_type == PNG_COLOR_TYPE_RGB_ALPHA) {
    if (channels != 4) {
      PS_LOG_DFATAL(handler,
                    "Encountered unexpected number of channels for RGBA "
                    "image: %d",
                    channels);
      return false;
    }
    ...
    if (channels != 2) {
      PS_LOG_DFATAL(handler,
                    "Encountered unexpected number of channels for Gray + "
                    "Alpha image: %d",
                    channels);
      return false;
    }
  ...
  // We currently detect alpha only for 8/16 bit Gray/TrueColor with
  // Alpha channel. Only 8 or 16 bit depths are supported for these
  // modes.
  if (bit_depth % 8 != 0) {
  ...
  for (png_uint_32 row = 0; row < height; ++row) {
    auto* row_bytes = static_cast<unsigned char*>(*(row_pointers + row));
...
bool PngScanlineReader::InitializeRead(const PngReaderInterface& reader,
                                       const std::string& in, bool* is_opaque) {
  if (!read_.valid()) {
...
PngScanlineReader::~PngScanlineReader() = default;
...
PngScanlineReaderRaw::~PngScanlineReaderRaw() = default;
...
ScanlineStatus PngScanlineReaderRaw::InitializeWithStatus(
  ...
  // Set up transformations. We will transform the input to one of
  // these formats: GRAY_8, RGB_888, and RGBA_8888.
  //
  // Reference for setting up transformations is in the
  // png_read_png() function in pngread.c.
  //
  // Strip 16 bit per color down to 8 bits per color.
  png_set_strip_16(png_ptr);

  // Expand grayscale images to full 8 bits from 1, 2, or 4 bits per
  // pixel. Expand paletted or RGB images with transparency to full
  // alpha channels so the data will be available as RGBA quartets.
  ...
  if (color_type == PNG_COLOR_TYPE_GRAY_ALPHA ||
    ...
    // Gray_Alpha by png_set_expand) to RGBA.
  ...
  // Guard against integer overflow and absurd allocation sizes before any
  // buffer is allocated. An interlaced (progressive) PNG buffers the entire
  // decoded image (bytes_per_row_ * height_) up front in
  // ReadNextScanlineWithStatus, so a malicious image whose dimensions are
  // within libpng's own limits could otherwise drive a multi-terabyte
  // allocation.
  size_t total_bytes = 0;
  if (!ValidateDecodedSize(
          width_, height_,
          GetNumChannelsFromPixelFormat(pixel_format_, message_handler_),
          &bytes_per_row_, &total_bytes)) {
    png_struct_.reset();
    return PS_LOGGED_STATUS(
        PS_LOG_ERROR, message_handler_, SCANLINE_STATUS_INTERNAL_ERROR,
        SCANLINE_PNGREADERRAW, "image too large to decode safely");
  }
  row_ = 0;
...
bool PngScanlineReaderRaw::ValidateDecodedSize(size_t width, size_t height,
                                               size_t channels,
  ...
  // bytes_per_row = width * channels. Checking against the (much smaller than
  // SIZE_MAX) byte cap also precludes any multiplication overflow.
  if (width > kMaxDecodedImageBytes / channels) {
    return false;
  }
  size_t bpr = width * channels;
  // total = bytes_per_row * height (the interlaced full-image buffer).
  if (bpr != 0 && height > kMaxDecodedImageBytes / bpr) {
    return false;
  }
  *bytes_per_row = bpr;
  *total_bytes = bpr * height;
  return true;
}
...
ScanlineStatus PngScanlineReaderRaw::ReadNextScanlineWithStatus(
  ...
  // In case libpng has an error, program will jump to the following
  // 'setjmp', which will have value of non-zero. To clean up memory
  // properly, we have to define row_pointers before 'setjmp' and
  // clean it up when error happens.
  if (setjmp(png_jmpbuf(png_ptr)) != 0) {
    Reset();
    return PS_LOGGED_STATUS(
        PS_LOG_INFO, message_handler_, SCANLINE_STATUS_INTERNAL_ERROR,
  ...
  // At the first time when ReadNextScanline() is called, we allocate
  // buffer to store the decoded pixels. For non-progressive
  // (non-interlacing) image, we need buffer to store a row of pixels.
  // For progressive image, we need buffer to store the entire image.
  // For progressive image, we also decode the entire image at the
  // first call.
  if (row_ == 0) {
    if (!is_progressive_) {
      ...
      // For a progressive PNG, we have to decode the entire image
      // before rendering any row. So at the first time when
      // ReadNextScanline() is called, we decode the entire image
      // into image_buffer_.
      if (image_buffer_ != nullptr) {
        // Initialize an array of pointers, which specify the
        // address of rows.
        ...
        // Decode the entire image. The results are stored in
        // image_buffer_.
  ...
  if (!is_progressive_) {  // NOLINT(bugprone-branch-clone): then-branch calls png_read_row; structurally different.
    // For a non-progressive PNG, we decode the image a row at a
    // time.
    png_read_row(png_ptr, image_buffer_.get(), nullptr);
    *out_scanline_bytes = static_cast<void*>(image_buffer_.get());
  } else {
    // For a progressive PNG, we simply point the output to the
    // corresponding row, because the image has already been decoded.
    *out_scanline_bytes =
...
bool PngScanlineWriter::Validate(const PngCompressParams* params,
                                 std::string* png_image) {
  if (params != nullptr) {
    // PNG_NO_FILTERS == 0
    // PNG_ALL_FILTERS == (PNG_FILTER_NONE | PNG_FILTER_SUB |
    ...
    if (params->filter_level & (~PNG_ALL_FILTERS)) {
      PS_LOG_DFATAL(message_handler_,
                    ...
                    "or bitwise OR of some of them: PNG_NO_FILTERS, "
    ...
    switch (params->compression_strategy) {
      ...
      default:
        PS_LOG_DFATAL(message_handler_,
                      "Compression strategy must be one of the following "
                      "values: Z_DEFAULT_STRATEGY, Z_FILTERED, "
...
ScanlineStatus PngScanlineWriter::InitWithStatus(const size_t width,
  ...
  size_t bpp = GetBytesPerPixel(pixel_format);
  if (bpp > 0 && width > SIZE_MAX / bpp) {
    return PS_LOGGED_STATUS(PS_LOG_ERROR, message_handler_,
                            SCANLINE_STATUS_INTERNAL_ERROR, SCANLINE_PNGWRITER,
                            "width too large for scanline allocation");
  }
  bytes_per_row_ = width * bpp;
  return ScanlineStatus(SCANLINE_STATUS_SUCCESS);
}

// Initialize the basic parameter for writing the image. To use the
// default compression parameters, set 'params' to NULL.
ScanlineStatus PngScanlineWriter::InitializeWriteWithStatus(
    const void* const params, std::string* const png_image) {
...
bool PngScanlineWriter::DoBestCompression() {
  std::string* png_image =
      static_cast<std::string*>(png_get_io_ptr(png_struct_->png_ptr()));
  ...
  // Both locals live across the setjmp below: after a longjmp, the values
  // of non-volatile locals modified since setjmp are indeterminate (and gcc
  // enforces this with -Werror=clobbered on host builds).
  volatile int min_size = png_image->length();
  volatile size_t i = 0;
  while (i < kParamCount) {
