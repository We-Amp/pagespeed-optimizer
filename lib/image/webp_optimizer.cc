...
// WebP image codec implementation: reader, writer, and animated
// WebP writer.
// Ported from mod_pagespeed's
// pagespeed/kernel/image/webp_optimizer.cc
...
using net_instaweb::FRAME_WEBPWRITER;
...
using net_instaweb::SCANLINE_STATUS_TIMEOUT_ERROR;
using net_instaweb::SCANLINE_STATUS_UNSUPPORTED_FEATURE;
using net_instaweb::SCANLINE_WEBPREADER;
...
static const char* const kWebPErrorMessages[] = {
    ...
    "BITSTREAM_OUT_OF_MEMORY: Out of memory re-allocating byte "
    "buffer",
...
// Fill a rectangular area within image with color. Caller must clip
// rectangle to within image.
...
// Copy rectangular region of pixels from src to dst. Caller must
// ensure regions are within src and dst.
...
// Disposes the previous frame if necessary. Called prior to drawing
// frame.
bool DisposeImage(const FrameSpec* frame, const FrameSpec* previous_frame,
  ...
  switch (frame->disposal) {
    case FrameSpec::DISPOSAL_RESTORE:
      // The current frame will need disposed by restoring the last
      // non-disposed frame. Cache it now.
      if (*cache == nullptr) {
...
// Blend a single channel of 'src' over 'dst', given their alpha
// channel values.
...
uint32_t BlendPixel(uint32_t src, uint32_t dst) {
  ...
  if (src_a == 255) {
    ...
    // This is the approximate integer arithmetic for the actual
    // formula: dst_factor_a = (dst_a * (255 - src_a)) / 255.
...
int StringWriter(const uint8_t* data, size_t data_size,
  ...
  auto* output = static_cast<std::string*>(picture->custom_ptr);
...
WebpConfiguration::~WebpConfiguration() = default;
...
ScanlineStatus WebpFrameWriter::Initialize(const void* config,
  ...
  const auto* webp_config = static_cast<const WebpConfiguration*>(config);
  ...
  if (webp_config->progress_hook != nullptr) {
...
int WebpFrameWriter::ProgressHook(int percent, const WebPPicture* picture) {
  ...
  return static_cast<int>(
      webp_writer->progress_hook_(percent, webp_writer->progress_hook_data_));
}
...
ScanlineStatus WebpFrameWriter::CacheCurrentFrame() {
  ...
  if (progress_hook_ != nullptr) {
    if (webp_image_.progress_hook != ProgressHook ||
        webp_image_.user_data != this) {
      return PS_LOGGED_STATUS(
          PS_LOG_DFATAL, message_handler(), SCANLINE_STATUS_INTERNAL_ERROR,
          FRAME_WEBPWRITER,
          "CacheCurrentFrame: progress hook or user data mismatch");
    }
  }
  const int current_time = timestamp_;
  // Guard against integer overflow when accumulating frame timestamps.
  // WebPAnimEncoderAdd requires int (32-bit signed) for timestamp_ms.
  // INT_MAX ms ≈ 24.8 days of animation — reject if exceeded.
  if (frame_spec_.duration_ms > static_cast<size_t>(INT_MAX - timestamp_)) {
    return PS_LOGGED_STATUS(
        PS_LOG_ERROR, message_handler(), SCANLINE_STATUS_PARSE_ERROR,
        FRAME_WEBPWRITER,
        "Animation timestamp overflow: total duration exceeds INT_MAX ms");
  }
  timestamp_ += static_cast<int>(frame_spec_.duration_ms);
...
ScanlineStatus WebpFrameWriter::WriteNextScanline(const void* scanline_bytes) {
  ...
  const auto* const in_bytes = reinterpret_cast<const uint8_t*>(scanline_bytes);
  if (!empty_frame_) {
    if (should_expand_gray_to_rgb_) {
      ...
      // once. However, since this is an "inner loop" iterating over
      // a series of pixels, we want to take advantage of the inline
...
ScanlineStatus WebpFrameWriter::FinalizeWrite() {
  ...
  if (image_spec_->num_frames <= 1) {
    ...
    PS_DLOG_INFO(message_handler(),
                 "Stats: coded_size: %d; lossless_size: %d; "
                 "alpha size: %d;",
...
// image_buffer must remain unchanged until the *first* call to
// ReadNextScanline().
ScanlineStatus WebpScanlineReader::InitializeWithStatus(
  ...
  // Guard against integer overflow in buffer allocation.
  if (width_ == 0 || height_ == 0) {
    Reset();
    return PS_LOGGED_STATUS(PS_LOG_ERROR, message_handler_,
                            SCANLINE_STATUS_UNSUPPORTED_FEATURE,
                            SCANLINE_WEBPREADER, "WebP dimensions are zero");
  }
  const size_t num_channels =
      GetNumChannelsFromPixelFormat(pixel_format_, message_handler_);
  if (num_channels == 0 || width_ > SIZE_MAX / num_channels) {
    Reset();
    return PS_LOGGED_STATUS(
        PS_LOG_ERROR, message_handler_, SCANLINE_STATUS_UNSUPPORTED_FEATURE,
        SCANLINE_WEBPREADER, "WebP width too large: overflow in bytes_per_row");
  }
  bytes_per_row_ = width_ * num_channels;
  if (height_ > SIZE_MAX / bytes_per_row_) {
    Reset();
    return PS_LOGGED_STATUS(
        PS_LOG_ERROR, message_handler_, SCANLINE_STATUS_UNSUPPORTED_FEATURE,
        SCANLINE_WEBPREADER,
        "WebP dimensions too large: overflow in pixel buffer size");
  }
...
ScanlineStatus WebpScanlineReader::ReadNextScanlineWithStatus(
  ...
  // The first time ReadNextScanline() is called, we decode the
  // entire image.
  if (row_ == 0) {
    // Overflow safety: bytes_per_row_ * height_ was validated in
    // InitializeWithStatus() — both factors and their product are
    // guaranteed not to overflow size_t.
    const size_t pixel_buffer_size = bytes_per_row_ * height_;
    pixels_.reset(new uint8_t[pixel_buffer_size]);
    if (pixels_ == nullptr) {
      Reset();
      return PS_LOGGED_STATUS(PS_LOG_ERROR, message_handler_,
    ...
    if (!WebPInitDecoderConfig(&config)) {
      return PS_LOGGED_STATUS(
          ...
          SCANLINE_WEBPREADER, "WebPInitDecoderConfig failed.");
    }
    ...
    config.output.u.RGBA.size = pixel_buffer_size;
  ...
  // Point output to the corresponding row of the already decoded
  // image.
  *out_scanline_bytes =
