...
// JPEG quality estimation implementation.
// Ported from mod_pagespeed's pagespeed/kernel/image/jpeg_utils.cc
...
static double ComputeQualityEntriesSum(JQUANT_TBL* quantization_table,
  ...
  // Quality is defined in terms of the base quantization tables used
  // by encoder. Q = quant table, q = compression quality and
  // S = table used by encoder, Encoder does the following.
  // if q > 0.5 then Q = 2 - 2*q*S otherwise Q = (0.5/q)*S.
  //
  // Refer 'jpeg_add_quant_table (...)' in jcparam.c for more details.
  //
  // Since we don't have access to the table used by encoder. But it
  // is generally close to the standard table defined by JPEG. Hence,
  // we apply inverse function of the above to using standard table
  // and compute the input image jpeg quality.
  for (int i = 0; i < DCTSIZE2; i++) {
    if (quantization_table->quantval[i] == 1) {
      // 1 is the minimum denominator allowed for any value in the
      // quantization matrix and it implies that quality is set 100.
...
int JpegUtils::GetImageQualityFromImage(const void* image_data,
  ...
  if (quality_entries_count > 0) {
    // This computed quality is in the form of a fraction, so
    // multiplying with 100 and rounding off to nearest integer.
