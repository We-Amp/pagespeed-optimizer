...
// Ported from mod_pagespeed's pagespeed/kernel/image/image_frame_interface.h

#ifndef PAGESPEED_LIB_IMAGE_IMAGE_FRAME_INTERFACE_H_
#define PAGESPEED_LIB_IMAGE_IMAGE_FRAME_INTERFACE_H_
...
struct FrameSpec {
  enum DisposalMethod : std::uint8_t {
    DISPOSAL_UNKNOWN = 0,
    DISPOSAL_NONE,
    DISPOSAL_BACKGROUND,
...
class MultipleFrameWriter {
 public:
  ...
  virtual ScanlineStatus Initialize(const void* config, std::string* out) = 0;
  ...
  bool Initialize(const void* config, std::string* out,
                  ScanlineStatus* status) {
...
#endif  // PAGESPEED_LIB_IMAGE_IMAGE_FRAME_INTERFACE_H_
