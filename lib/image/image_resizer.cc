...
ResizeTableEntry* CreateTableForAreaMethod(int in_size, int out_size,
  ...
  auto* table = new ResizeTableEntry[out_size];
  if (table == nullptr) {
  ...
  for (int i = 0; i < out_size; ++i) {
    ...
    if (i > 0 && table[i - 1].first_index >= table[i].first_index) {
      PS_LOG_DFATAL(handler,
                    "Significant rounding error has been accumulated.");
      return nullptr;
    }
  }
...
using net_instaweb::SCANLINE_RESIZER;
...
class ResizeCol {
 public:
  ...
  [[nodiscard]] virtual bool NeedMoreScanlines() const = 0;
  [[nodiscard]] virtual int out_row() const = 0;
};

// Base class for the horizontal resizer using the "area" method.
class ResizeRowArea : public ResizeRow {
 public:
  explicit ResizeRowArea(int num_channels) : num_channels_(num_channels) {}
 ...
 protected:
  ...
  int pixels_per_row_;
  float* output_buffer_{nullptr};  // Not owned
...
class ResizeColArea : public ResizeCol {
 public:
  ...
  [[nodiscard]] int out_row() const override { return out_row_; }

  void InitializeResize() override { need_more_scanlines_ = true; }

  [[nodiscard]] bool NeedMoreScanlines() const override {
 ...
 private:
  ...
  uint8_t* output_buffer_{nullptr};  // Not owned
...
bool ResizeColArea<BufferType>::Initialize(int in_size, int out_size,
  ...
  auto grid_area = static_cast<float>(ratio_x * ratio_y);
...
void ResizeColArea<BufferType>::ComputeOutput(const float* in_data,
  ...
  // Clamp to [0, 255] before casting to avoid undefined behavior when
  // rounding produces values outside the uint8_t range.
  auto clamp_u8 = [](float v) -> uint8_t {
    return static_cast<uint8_t>(std::min(255.0f, std::max(0.0f, v)));
  };
  for (; index < elements_per_row_4_; index += 4) {
    out_data[index] =
        clamp_u8((in_data[index] + half_grid_area) * inv_grid_area);
    out_data[index + 1] =
        clamp_u8((in_data[index + 1] + half_grid_area) * inv_grid_area);
    out_data[index + 2] =
        clamp_u8((in_data[index + 2] + half_grid_area) * inv_grid_area);
    out_data[index + 3] =
        clamp_u8((in_data[index + 3] + half_grid_area) * inv_grid_area);
  }
  for (; index < elements_per_row_; ++index) {
    out_data[index] =
        clamp_u8((in_data[index] + half_grid_area) * inv_grid_area);
...
const uint8_t* ResizeColArea<BufferType>::Resize(const void* in_data_ptr) {
  if (only_scale_outputs_) {
    ...
    if (output_buffer_ == nullptr) {
      ...
      const auto* in_data = static_cast<const float*>(in_data_ptr);
      ComputeOutput(in_data, output_buffer_);
      return output_buffer_;
    }
  }

  const auto* in_data = reinterpret_cast<const BufferType*>(in_data_ptr);
...
ScanlineResizer::~ScanlineResizer() = default;
...
ScanlineStatus ScanlineResizer::ReadNextScanlineWithStatus(
  ...
  while (resizer_y_->NeedMoreScanlines()) {
    ...
    // Resize the input scanline horizontally and put the results in
    // buffer_.
    const void* buffer =
...
// integer for internal computation and buffer if it is possible. In
// particular,
// - If both ratio_x and ratio_y are integers, use integer for all
//   computation;
...
bool ScanlineResizer::Initialize(ScanlineReaderInterface* reader,
  ...
  if (request_width == kPreserveAspectRatio &&
    ...
    PS_LOG_DFATAL(message_handler_,
                  "Output width and height cannot be "
                  "kPreserveAspectRatio "
                  "at the same time.");
    return false;
  }

  if (reader->GetImageWidth() > static_cast<size_t>(INT_MAX) ||
      reader->GetImageHeight() > static_cast<size_t>(INT_MAX)) {
    PS_LOG_DFATAL(message_handler_, "Image dimensions exceed INT_MAX.");
    return false;
  }
  const int input_width = static_cast<int>(reader->GetImageWidth());
  const int input_height = static_cast<int>(reader->GetImageHeight());

  // TODO(huibao): Truncate the requested image size if it is larger than
  // the input in 'image_rewrite_filter.cc'. Report an error and return
  // 'false' if it is larger than the input in this method.

  // If the request size for either dimension is greater than that of the
  // input, it will be truncated. In other words, the image will not be
  // enlarged.
  ...
  size_t epr = static_cast<size_t>(resized_width) * num_channels;
  if (epr > static_cast<size_t>(INT_MAX)) {
    PS_LOG_DFATAL(message_handler_, "Image row size exceeds INT_MAX.");
    return false;
  }
  elements_per_row_ = static_cast<int>(epr);

  // Ratios           | X Resizer | X Buff | Y Input | Y Resizer      |
  // Y Buff x != 1 && y != 1 | Resize    | Valid  | float   | Resize &
  // Scale | Valid x != 1 && y == 1 | Resize    | Valid  | float   |
  // Scale Only     | Valid x == 1 && y != 1 | Shortcut  | NULL   |
  // uint8   | Resize & Scale | Valid x == 1 && y == 1 | Shortcut  |
  // NULL   | uint8   | Shortcut       | NULL
