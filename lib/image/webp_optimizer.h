...
// WebP image codec: reader, writer, and animated WebP writer.
// Ported from mod_pagespeed's pagespeed/kernel/image/webp_optimizer.h

#ifndef PAGESPEED_LIB_IMAGE_WEBP_OPTIMIZER_H_
#define PAGESPEED_LIB_IMAGE_WEBP_OPTIMIZER_H_
...
struct WebpConfiguration : public ScanlineWriterConfig {
  ...
  int lossless;   // Lossless encoding (0=lossy, 1=lossless). The
                  // constructor defaults this to 1 (lossless), which
                  // suits palette/graphics sources (e.g. GIF frames).
                  // Callers encoding photographic sources MUST set
                  // this to 0 explicitly, or 'quality' acts as
                  // lossless compression effort instead of fidelity
                  // and the output is typically far larger. The
                  // default is retained (not flipped to 0) to keep
                  // the ported mod_pagespeed contract; see
                  // WebpConfigurationTest.DefaultValues.
  float quality;  // between 0 (smallest file) and 100 (biggest)
  int method;     // quality/speed trade-off (0=fast, 6=slower-better)

  // Parameters related to lossy compression only:
  int target_size;        // if non-zero, set the desired target size
                          // in bytes. Takes precedence over the
                          // 'compression' parameter.
  int alpha_compression;  // Algorithm for encoding the alpha plane
                          // (0 = none, 1 = compressed with WebP
                          // lossless). Default is 1.
  int alpha_filtering;    // Predictive filtering method for alpha
                          // plane. 0: none, 1: fast, 2: best.
                          // Default is 1.
  int alpha_quality;      // Between 0 (smallest size) and 100
  ...
  size_px kmin;  // Minimum keyframe interval, i.e., number of
                 ...
                 // requires kmax > kmin >= (kmax / 2) + 1.
                 // Reasonable choices are (3,5) for lossy encoding
                 // and (9,17) for lossless encoding.
  size_px kmax;  // Maximum keyframe interval.

  WebpProgressHook progress_hook;  // If non-NULL, called during
                                   // encoding.
...
class WebpFrameWriter : public MultipleFrameWriter {
 ...
 private:
  // The function to be called by libwebp's progress hook (with
  // 'this' as the user data), which in turn will call the
  // user-supplied function in progress_hook_, passing it
  // progress_hook_data_.
  ...
  // Zero-based index of the next frame (after the current one) to
  // be written.
  size_px next_frame_;
  ...
  // least one of its dimensions being zero. Note that all frames
  // must fit completely within their image (see the comment in
  ...
  // Pointer to the next pixel to be written via
  // WriteNextScanline(). If the frame is offset (top or left != 0),
  // this pointer is also offset.
  ...
  // The user-supplied user data for progress_hook. This pointer
  // must remain valid until FinalizeWrite() completes. This class
  // does NOT take ownership of this pointer.
  void* progress_hook_data_;

  // WebP does not have native support for gray scale images. The
  // workaround is to replicate the luminance to RGB; then WebP can
  // compress the expanded images efficiently.
  bool should_expand_gray_to_rgb_;

  // Min and max keyframe interval values. Only applicable for
  // animated webp.
  size_px kmin_;
  size_px kmax_;
...
// WebpScanlineReader decodes WebP images. It returns a scanline
// (a row of pixels) each time it is called. The output format is
// RGB_888 if the input image does not have alpha channel, or
// RGBA_8888 otherwise. Animated WebP is not supported.
class WebpScanlineReader : public ScanlineReaderInterface {
 public:
  ...
  // Return the next row of pixels. The entire image is decoded the
  // first time ReadNextScanline() is called, but only one scanline
...
#endif  // PAGESPEED_LIB_IMAGE_WEBP_OPTIMIZER_H_
