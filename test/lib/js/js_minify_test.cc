...
class JsMinifyTest : public testing::Test {
 protected:
  void CheckNewMinification(std::string_view before, std::string_view after) {
    std::string output;
  ...
  void CheckMinification(std::string_view before, std::string_view after) {
    CheckNewMinification(before, after);
  }

  void CheckNewError(std::string_view input) {
    std::string output;
    EXPECT_FALSE(pagespeed::js::MinifyUtf8Js(&patterns_, input, &output));
  }

  void CheckError(std::string_view input) { CheckNewError(input); }
  ...
  void CheckFileMinification(std::string_view before_filename,
                             std::string_view after_filename) {
    std::string original = ReadTestFile(before_filename);
    ASSERT_FALSE(original.empty()) << "Failed to read: " << before_filename;
    std::string expected = ReadTestFile(after_filename);
    ASSERT_FALSE(expected.empty()) << "Failed to read: " << after_filename;
    std::string actual;
...
TEST_F(JsMinifyTest, Json) {
  CheckMinification(R"({  "foo": {"bar": 1}, "baz": 2 } )",
                    R"({"foo":{"bar":1},"baz":2})");
}
...
TEST_F(JsMinifyTest, RetainedCommentNoRegexGlue) {  // issue #1022
  CheckNewMinification("var await = 1;\nx = await / 2 / /*@ c @*/ y;",
                       "var await=1;x=await/ 2 / /*@ c @*/y;");
  CheckNewMinification("x = a / /*@c@*/ b;", "x=a/ /*@c@*/b;");
}

TEST_F(JsMinifyTest, RetainedCommentTrailingSlashNoGlue) {  // issue #1022
...
TEST_F(JsMinifyTest, RetainedCommentPreservesAsiLinebreak) {  // issue #1022
  CheckNewMinification("return\n/*@c@*/ 42;", "return\n/*@c@*/42;");
}

TEST_F(JsMinifyTest, RetainedCommentPreservesAsiAfterComment) {  // issue #1022
...
TEST_F(JsMinifyTest, RegexLiteralWithBrackets1) {
  ...
  CheckMinification(R"(var x = /http:\/\/[^/]+\//, y = 3;)",
                    R"(var x=/http:\/\/[^/]+\//,y=3;)");
}
...
TEST_F(JsMinifyTest, AwaitRegex) {
  ...
  CheckNewMinification("async function f(){ return await /a +b/.test(s); }",
                       "async function f(){return await/a +b/.test(s);}");
  CheckNewMinification("async () => { await / x /g.test(s); };",
...
TEST_F(JsMinifyTest, ForOfRegex) {
  ...
  CheckNewMinification("for (const m of / x +y /.exec(s)) m();",
...
TEST_F(JsMinifyTest, AwaitYieldOfAsOrdinaryIdentifiers) {
  ...
  CheckNewMinification("x.await / 2;", "x.await/2;");
  CheckMinification("x.of / 2;", "x.of/2;");
  CheckNewMinification("x.yield / 2;", "x.yield/2;");
...
TEST_F(JsMinifyTest, SpeculativeRegexGuardNarrowness) {
  ...
  CheckMinification("async function f(){ return await /[/*]/.test(x) }",
                    "async function f(){return await/[/*]/.test(x)}");
  CheckNewMinification("for (m of /[//]/.exec(s)) m();",
                       "for(m of/[//]/.exec(s))m();");
...
TEST_F(JsMinifyTest, SpeculativeOperatorBlockGuard) {
  ...
  CheckNewMinification("class await extends B {}", "class await extends B{}");
...
TEST_F(JsMinifyTest, AwaitAsSeparatedIdentifier) {
  ...
  CheckNewMinification("async function f(){ return { a: await g(), b: 1 }; }",
                       "async function f(){return{a:await g(),b:1};}");
  CheckNewMinification("async function f(){ await g(), await h(); }",
...
TEST_F(JsMinifyTest, KeywordPrecedesRegex) {
  ...
  for (pagespeed::JsKeywords::Iterator iter; !iter.AtEnd(); iter.Next()) {
    if (pagespeed::JsKeywords::CanKeywordPrecedeRegEx(iter.name())) {
      std::string input = std::string(iter.name()) + " /./   /* hi there */;";
      std::string expected = std::string(iter.name()) + "/./;";
...
// SemicolonInsertionFuncCall above).
...
// Proof test: verify that the E2E test file (app.js) is significantly
// reduced by MinifyUtf8Js.  If this test passes, then E2E test_js_minified_on_hit
// failures are timing issues, not minifier bugs.
TEST_F(JsMinifyTest, MinifiesE2eAppJs) {
  // Exact contents of tools/e2e/testdata/app.js (769 bytes with newline).
  const char kAppJs[] =
  ...
  std::string output;
  ASSERT_TRUE(pagespeed::js::MinifyUtf8Js(&patterns_, kAppJs, &output));
  // The file has extensive whitespace and comments — minification should
  // reduce it significantly (original ~769B, expected output < 700B).
  EXPECT_LT(output.size(), 700u)
      << "Minified output too large: " << output.size() << " bytes";
  // Key identifier must survive minification.
  EXPECT_NE(output.find("initApp"), std::string::npos)
      << "initApp missing from minified output";
}

TEST_F(JsMinifyTest, KeywordsIteratorKeyword) {
  // Exercise JsKeywords::Iterator::keyword() (not just name())
  pagespeed::JsKeywords::Iterator iter;
  int count = 0;
  while (!iter.AtEnd()) {
    auto type = iter.keyword();
    EXPECT_NE(type, pagespeed::JsKeywords::kNotAKeyword);
    auto* name = iter.name();
...
// Block comments containing line terminators must preserve ASI semantics.
// A newline inside a block comment acts as a line terminator for semicolon
// insertion purposes.
TEST_F(JsMinifyTest, BlockCommentNewlinePreservesASI) {
  // return/*\n*/42 must preserve the linebreak: return\n42 (not return 42).
  CheckMinification("return/*\n*/42", "return\n42");
  // Same with \r.
  CheckMinification("return/*\r*/42", "return\n42");
  // Same with \r\n.
  CheckMinification("return/*\r\n*/42", "return\n42");
  // Block comment without newline: just a space (return 42 would be ok
  // because return + space + 42 is fine semantically, but minifier merges
  // to "return 42").
  CheckMinification("return/* */42", "return 42");
}

TEST_F(JsMinifyTest, BlockCommentNewlineThrow) {
  // throw/*\n*/'error' must preserve the linebreak.
  CheckMinification("throw/*\n*/'error'", "throw\n'error'");
}

// Bare 0 is a decimal literal that can absorb a decimal point, so
// "0 .toString()" must preserve the space before the period.
TEST_F(JsMinifyTest, BareZeroDotPreservesSpace) {
  CheckNewMinification("0 .toString()", "0 .toString()");
  // 00 is octal, so no space needed.
  CheckNewMinification("00 .toString()", "00.toString()");
}

// After "else", a "/" starts a regex literal, not division.
TEST_F(JsMinifyTest, ElseRegexNewMinifier) {
  CheckNewMinification("if(x)foo()\nelse /bar/.test(s)",
                       "if(x)foo()\nelse/bar/.test(s)");
}

// =============================================================================
// Template literal support: verify template literals survive minification
// via MinifyUtf8Js (the new tokenizer path).
// =============================================================================

TEST_F(JsMinifyTest, TemplateLiteralSurvivesMinification) {
  // Simple template literal with surrounding whitespace.
  CheckNewMinification("var  x  =  `hello world`;", "var x=`hello world`;");
}

TEST_F(JsMinifyTest, TemplateLiteralInterpolationSurvivesMinification) {
  // Template literal with ${} interpolation — the entire literal (including
  // interpolation) is treated as a single string token by the tokenizer.
  CheckNewMinification("var  msg  =  `hello ${name}!`;",
                       "var msg=`hello ${name}!`;");
}

TEST_F(JsMinifyTest, TemplateLiteralMultilineSurvivesMinification) {
  // Template literals preserve internal newlines.
  CheckNewMinification("var  x  =  `line1\nline2`;", "var x=`line1\nline2`;");
}

TEST_F(JsMinifyTest, TemplateLiteralNestedBracesSurvivesMinification) {
  // Nested braces inside interpolation.  Re-baselined for the adopted 1.15
  // kernel: a template literal is no longer one opaque token.  It is split
  // into head/middle/tail chunks (kTemplateLiteral) with the JavaScript
  // inside each ${...} tokenized as ordinary tokens, so the interpolation
  // body is now minified too.  The literal text outside ${...} is still
  // preserved byte-for-byte.
  CheckNewMinification("var  x  =  `${a ? {b: 1} : {c: 2}}`;",
                       "var x=`${a?{b:1}:{c:2}}`;");
}

TEST_F(JsMinifyTest, UnterminatedTemplateLiteralIsError) {
  // Unterminated template literal should produce an error.
  CheckNewError("`hello world");
}

// =============================================================================
// ES6 keyword support: verify modern JS with class/import/export minifies
// without error via MinifyUtf8Js.
// =============================================================================

TEST_F(JsMinifyTest, ES6ExportFunctionMinification) {
  // 'export' keyword is recognized and does not cause an error when followed
  // by a function declaration.
  CheckNewMinification("export  function  foo()  {  return  42;  }",
                       "export function foo(){return 42;}");
}

TEST_F(JsMinifyTest, ES6ConstLetMinification) {
  // The minifier collapses the newline between semicolon-terminated statements.
  CheckNewMinification("const  x  =  1;\nlet  y  =  2;", "const x=1;let y=2;");
}

TEST_F(JsMinifyTest, ES6ArrowFunctionMinification) {
  CheckNewMinification("const  fn  =  ( x )  =>  x  +  1;",
                       "const fn=(x)=>x+1;");
}

TEST_F(JsMinifyTest, ES6ImportExportMinification) {
  // import/export are reserved keywords — they should be recognized and
  // preserved by the minifier (not treated as identifiers that could be
  // collapsed). This tests that the minifier does not error on them.
  std::string input = "export  function  foo()  {  return  42;  }";
  std::string output;
  EXPECT_TRUE(pagespeed::js::MinifyUtf8Js(&patterns_, input, &output));
  // The output should contain 'export' and 'function' and 'foo'.
  EXPECT_NE(std::string::npos, output.find("export"));
  EXPECT_NE(std::string::npos, output.find("function"));
  EXPECT_NE(std::string::npos, output.find("foo"));
  // Should be smaller than input (whitespace removed).
  EXPECT_LT(output.size(), input.size());
}

// =============================================================================
// Nullish coalescing (??) and optional chaining (?.) minification tests.
// These operators were previously unrecognized, causing parse errors.
// =============================================================================

TEST_F(JsMinifyTest, NullishCoalescingMinification) {
  // var x = a ?? b; should minify without error.
  CheckNewMinification("var  x  =  a  ??  b;", "var x=a??b;");
}

TEST_F(JsMinifyTest, NullishCoalescingChainedMinification) {
  // Chained: a ?? b ?? c
  CheckNewMinification("var  x  =  a  ??  b  ??  c;", "var x=a??b??c;");
}

TEST_F(JsMinifyTest, OptionalChainingPropertyMinification) {
  // obj?.prop should minify without error.
  CheckNewMinification("var  x  =  obj  ?.  prop;", "var x=obj?.prop;");
}

TEST_F(JsMinifyTest, OptionalChainingMethodMinification) {
  // obj?.method() should minify without error.
  CheckNewMinification("var  x  =  obj  ?.  method ( );",
                       "var x=obj?.method();");
}

TEST_F(JsMinifyTest, OptionalChainingChainedMinification) {
  // a?.b?.c should minify without error.
  CheckNewMinification("var  x  =  a  ?.  b  ?.  c;", "var x=a?.b?.c;");
}

TEST_F(JsMinifyTest, NullishCoalescingWithTernaryMinification) {
  // a ?? b ? c : d — ?? followed by ternary should not confuse the minifier.
  CheckNewMinification("var  x  =  a  ??  b  ?  c  :  d;", "var x=a??b?c:d;");
}

// =============================================================================
// Arrow-function block bodies '=>{...}' and optional catch binding 'catch{}'.
// The deliberately-minimal tokenizer previously misparsed the '{' after '=>'
// as an object literal (so any statement keyword inside errored), and rejected
// the ES2019 optional catch binding 'catch{}' (no parens).  See Issue D.
// =============================================================================

TEST_F(JsMinifyTest, ArrowBlockBodyTryCatch) {
  // Arrow function with a block body containing a try/catch statement.
  CheckNewMinification("x( ( ) => { try { a ( ) } catch ( e ) { b ( ) } } )",
                       "x(()=>{try{a()}catch(e){b()}})");
}

TEST_F(JsMinifyTest, ArrowBlockBodyIf) {
  // Arrow function whose block body contains an if-statement.
  CheckNewMinification("var f = ( ) => { if ( a ) { b ( ) } }",
                       "var f=()=>{if(a){b()}}");
}

TEST_F(JsMinifyTest, ArrowBlockBodyMultiStatementWithFor) {
  // Arrow block body with a for-loop.
  CheckNewMinification("( ) => { for ( ; ; ) { } }", "()=>{for(;;){}}");
}

TEST_F(JsMinifyTest, OptionalCatchBinding) {
  // ES2019 optional catch binding: catch with no parameter list.
  CheckNewMinification("try { a ( ) } catch { b ( ) }", "try{a()}catch{b()}");
}

TEST_F(JsMinifyTest, QuickMessageArrowBlock) {
  // The exact prod construct from QuickMessage that failed to minify: an
  // arrow block body with an optional catch binding and a comma sequence.
  CheckNewMinification(
      "n.addEventListener ( 'input' , ( ) => { try { s ( ) } catch { } y ( ) ,"
      " g ( ) } )",
      "n.addEventListener('input',()=>{try{s()}catch{}y(),g()})");
}

// Round-trip guards: arrow awareness must not regress object-literal, regex,
// or expression-body handling.

TEST_F(JsMinifyTest, ArrowReturningObjectLiteral) {
  // Parenthesized object literal as an arrow expression body: ()=>({}).
  CheckNewMinification("var f = ( ) => ( { } ) ;", "var f=()=>({});");
}

TEST_F(JsMinifyTest, ArrowReturningPopulatedObjectLiteral) {
  CheckNewMinification("var f = x => ( { a : 1 , b : 2 } ) ;",
                       "var f=x=>({a:1,b:2});");
}

TEST_F(JsMinifyTest, NestedArrowBlockBodies) {
  // An arrow whose block body returns another arrow with a block body.
  CheckNewMinification("var f = ( ) => { return ( ) => { return 1 } } ;",
                       "var f=()=>{return()=>{return 1}};");
}

TEST_F(JsMinifyTest, ArrowExpressionBodyStillMinifies) {
  // Expression-body arrows must still collapse correctly (no regression).
  CheckNewMinification("const  g  =  ( a , b )  =>  a  +  b ;",
                       "const g=(a,b)=>a+b;");
}

TEST_F(JsMinifyTest, SlashAfterArrowBlockIsRegex) {
  // A slash after an arrow block body's close brace begins a regex literal
  // (an arrow function is an expression, so really division — but the prior
  // tokenizer treated the block as a block; the safe expectation is that this
  // round-trips without error).  Use a regex inside the block to exercise the
  // block-vs-object decision directly.
  CheckNewMinification("var f = ( ) => { return / x /g } ;",
                       "var f=()=>{return/ x /g};");
}

TEST_F(JsMinifyTest, OptionalChainingCall) {
  // a?.() — optional call.  After ?. the next token is '(' directly.
  CheckNewMinification("var  x  =  a  ?.  ( ) ;", "var x=a?.();");
}

TEST_F(JsMinifyTest, OptionalChainingDynamicIndex) {
  // a?.[i] — optional dynamic index.  After ?. the next token is '[' directly.
  CheckNewMinification("var  x  =  a  ?.  [ i ] ;", "var x=a?.[i];");
}

TEST_F(JsMinifyTest, OptionalChainingMethodChainCall) {
  // The QuickMessage shape: obj?.closest?.(sel).
  CheckNewMinification("var  x  =  e . target ?. closest ?. ( s ) ;",
                       "var x=e.target?.closest?.(s);");
}

// ===========================================================================
// Destructuring binding patterns in let/const/var declarations.
//
// Before the kBindingDeclaration fix, the tokenizer-based minifier
// returned an error for "{"/"[" directly after const/var/let in a declaration,
// so the worker served such bundles unminified.  These end-to-end tests assert
// the minifier now produces correct minified output.  The corruption-safety
// invariant is covered by the guard tests below (object literals, "{let:1}"
// labels, etc. are unchanged).
// ===========================================================================

TEST_F(JsMinifyTest, DestructuringConstObject) {
  CheckNewMinification("const {a} = f();", "const{a}=f();");
}

TEST_F(JsMinifyTest, DestructuringVarObject) {
  CheckNewMinification("var {a, b} = f();", "var{a,b}=f();");
}

TEST_F(JsMinifyTest, DestructuringConstArray) {
  CheckNewMinification("const [a, b] = f();", "const[a,b]=f();");
}

TEST_F(JsMinifyTest, DestructuringRenamedProperties) {
  CheckNewMinification("const {a: x, b: y} = f();", "const{a:x,b:y}=f();");
}

TEST_F(JsMinifyTest, DestructuringLetObject) {
  CheckNewMinification("let {a, b} = f();", "let{a,b}=f();");
}

TEST_F(JsMinifyTest, DestructuringDefaultValues) {
  CheckNewMinification("let {a = {}, b = []} = f();", "let{a={},b=[]}=f();");
}

TEST_F(JsMinifyTest, DestructuringNested) {
  CheckNewMinification("const {a: {b}} = f();", "const{a:{b}}=f();");
}

TEST_F(JsMinifyTest, DestructuringComputedKey) {
  CheckNewMinification("let {[k]: v} = o;", "let{[k]:v}=o;");
}

TEST_F(JsMinifyTest, DestructuringForOfConst) {
  CheckNewMinification("for (const {a} of xs) { g(a); }",
                       "for(const{a}of xs){g(a);}");
}

TEST_F(JsMinifyTest, DestructuringMultipleDeclarators) {
  CheckNewMinification("const {a} = x, {b} = y;", "const{a}=x,{b}=y;");
}

TEST_F(JsMinifyTest, DestructuringArrayRest) {
  CheckNewMinification("const [a, ...b] = f();", "const[a,...b]=f();");
}

TEST_F(JsMinifyTest, DestructuringObjectRest) {
  CheckNewMinification("const {a, ...b} = f();", "const{a,...b}=f();");
}

TEST_F(JsMinifyTest, DestructuringInCatchBlock) {
  // The real production shape from modpagespeed.com's /analyze bundle: a "let"
  // object pattern immediately after "{" inside a catch block.
  CheckNewMinification(
      "try { g(); } catch (e) { let {title: t, body: n} = Oe(e); De(t, n); }",
      "try{g();}catch(e){let{title:t,body:n}=Oe(e);De(t,n);}");
}

// --- Corruption-safety guards: these inputs must be UNCHANGED by the fix. ----

TEST_F(JsMinifyTest, DestructuringGuardObjectLiteralUnchanged) {
  // A plain object literal in a "let" initializer must still minify normally
  // (the next char after "let" is an identifier, not "{"/"[").
  CheckNewMinification("let o = {a: 1, b: 2};", "let o={a:1,b:2};");
}

TEST_F(JsMinifyTest, DestructuringGuardDestructuredParamUnchanged) {
  // Destructured function parameters were already handled; still are.
  CheckNewMinification("function g({a, b}) {}", "function g({a,b}){}");
}

TEST_F(JsMinifyTest, DestructuringGuardLetLabelBlockUnchanged) {
  // "{let: 1}" at statement position is a block with the label "let:", NOT a
  // binding.  Must minify exactly as before (never corrupted into a decline of
  // a different construct).
  CheckNewMinification("{let: 1}", "{let:1}");
}

TEST_F(JsMinifyTest, DestructuringGuardLetPropertyKeyUnchanged) {
  // "let" as an object-literal property key in expression position.
  CheckNewMinification("x = {let: 1};", "x={let:1};");
}
...
TEST_F(JsMinifyTest, DeeplyNestedInputFailsGracefully) {
  ...
  CheckNewError(std::string(100000, '['));
  CheckNewError(std::string(100000, '('));
}

TEST_F(JsMinifyTest, Es2020NullishCoalescing) {
  CheckNewMinification("a ?? b;", "a??b;");
  // Split string literals: "??=" and "??/" are C++ trigraph sequences and
  // the build treats -Wtrigraphs as an error.
  CheckNewMinification(
      "x ?"
      "?= y;",
      "x?"
...
TEST_F(JsMinifyTest, Es2015LetBindingLinebreak) {
  // The binding lookahead is what makes the linebreak after a `let` binding
  // survive.  On the identifier reading `let row` is two identifiers, so
  // `row\n+4` looks like one continued expression and the linebreak is
  // droppable; on the (correct) declaration reading `let row` is a complete
  // lexical declaration that cannot continue with `+`, so ASI fires and the
  // linebreak is load-bearing.  Dropping it emits UNPARSEABLE output for
  // valid input.
  CheckNewMinification("let row\n+4;", "let row\n+4;");
  CheckNewMinification("let row\n+row;", "let row\n+row;");
  CheckNewMinification("function f() {\n let row\n+4;\n}",
                       "function f(){let row\n+4;}");
  // Controls: an initializer or a declaration-comma continues the
  // declaration, so the binding-linebreak question never arises.
  CheckNewMinification("let row = 1;", "let row=1;");
  CheckNewMinification("let a = 1, b = 2;", "let a=1,b=2;");
  // ...and the linebreak is kept when the next statement is not an
  // expression continuation either.
  CheckNewMinification("let row\nrow = 1;", "let row\nrow=1;");
}

TEST_F(JsMinifyTest, Es2015LetCommentBeforeBinding) {
  // RC-E regression battery (mpp #691 confirmation-nightly triage): a
...
TEST_F(JsMinifyTest, Es2015LetHtmlCommentBeforeBinding) {
  // Same RC-E defect as above, for the HTML comment forms (mpp #866, found
...
TEST_F(JsMinifyTest, Es2015LetHtmlCommentIdempotent) {
  // The minimized fuzz artifact for mpp #866: with the HTML forms missing
...
TEST_F(JsMinifyTest, LinebreakBeforeArrowHead) {  // issue #657
...
TEST_F(JsMinifyTest, ObjectSpreadCallElement) {  // issue #877
...
TEST_F(JsMinifyTest, GeneratorMethodCommaSeparator) {  // issue #658
...
// --- Source-map mapping tuples (consumer golden for MinifyUtf8JsWithSourceMap,
// --- ported from the canonical 1.15 test suite).

// Serializes Mappings so they can be compared against gold versions.
std::string MappingsToString(
    const net_instaweb::source_map::MappingVector& mappings) {
  std::string result("{");
  for (const net_instaweb::source_map::Mapping& m : mappings) {
    result += "(" + std::to_string(m.gen_line) + ", " +
              std::to_string(m.gen_col) + ", " + std::to_string(m.src_file) +
              ", " + std::to_string(m.src_line) + ", " +
              std::to_string(m.src_col) + "), ";
  }
  result += "}";
  return result;
}
