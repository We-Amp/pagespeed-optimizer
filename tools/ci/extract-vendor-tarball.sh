...
#
# Why this script exists (a recurring 1.1-line issue; ported to the optimizer):
...
#      remote tarball so the CI hub's age-based cleanup measures "time since
#      last consumed" rather than "time since vendored". A workflow rerun
#      within the window keeps its artifact alive without weakening cleanup
#      for idle repos. The sidecar is only touched when it already exists:
#      `touch` on an absent path CREATES an empty file, and an empty sidecar
#      both disables the digest check below (an unreadable want-line skips
#      it) and accumulates as an orphan once its tarball is pruned.
...
#   the remote file's mtime so the hub's retention window measures
#   last-consumed.
  ...
  for attempt in $(seq 1 "$FETCH_RETRIES"); do
    ...
    if rsync -a -e "ssh ${SSH_OPTS}" "$REMOTE_PATH" "$dest_path"; then
      if verify_tarball "$dest_path"; then
        # Extend in-flight TTL: re-stamp mtime so hub cleanup counts from
...
if [ -n "$CHOSEN" ]; then
  ...
  # A failed extraction can leave a partial tree behind; clear it so the hub
  # copy lands in a clean $DEST (documented divergence, see the SYNC NOTE).
