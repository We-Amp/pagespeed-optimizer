...
The server-layer module gives you automatic WebP and opt-in AVIF, content-negotiated and cached, installed from a signed package with no build step. For SVG auto-vectorization and a wider variant matrix — viewport class, pixel density, and Save-Data alongside format — [ModPageSpeed 2.0](/) runs the same lineage as an nginx reverse-proxy worker. See the [product page](/) for the supported servers and the [feature list](/features/) for what else the module rewrites beyond images.
