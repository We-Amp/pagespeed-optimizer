...
description: 'Optimizer worker HTTP API reference for mod_pagespeed 2.1: health, stats, Prometheus metrics, cache management, live config, and WebSocket streaming endpoints — all callable with curl.'
order: 61
group: 'Reference'
...
or the module) as content is served, so they reflect
...
optimizable into exactly one class each; today only the module
classifies serves, so under another front end the block reads all-zero —
