# Upgrading from mod_pagespeed 1.x to ModPageSpeed 2.0

This document covers the architectural differences, configuration migration, and
deployment changes when moving from mod_pagespeed 1.x (or ngx_pagespeed) to
ModPageSpeed 2.0.
...
starts cold and warms as traffic flows through.
...
filter there, unchanged). It covers an upgrade from a 2.0 host package or from
an earlier 2.1 release candidate.
...
Yes. They share no state. Run them on different servers or different nginx server blocks.

**Will my 1.x cache carry over?**
No. The Cyclone cache format is incompatible. The cache starts cold.

**Do I need to change my application code?**
No. All optimization is transparent at the reverse-proxy level.

**What about Apache?**
ModPageSpeed 2.0 uses nginx as its interceptor. Deploy the nginx + worker stack in front
of your Apache server as a reverse proxy. Your Apache configuration does not change.

**What if I need a filter that was removed?**
The removed filters (combining, spriting, JS-based lazy loading) addressed HTTP/1.1
limitations. If you still need them, keep running 1.x alongside 2.0. For most
deployments on HTTP/2, the 2.0 optimizations produce better results.
